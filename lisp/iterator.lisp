...
iterator. Length of zipper-iter is equal to length of shorter iterator.
...
(:fn init (self in-fst in-scnd)
     (do
       (if (iter? in-fst)
           ...
           (if (seq? in-fst))
            (set! fst (iter in-fst))
            (err "first argument to zipper must be iterator or vector"))
       (if (iter? in-scnd)
           (set! scnd in-scnd)
           (if (seq? in-scnd))
            (set! scnd (iter in-scnd))
            (err "second argument to zipper must be iterator or vector"))
       self))
(:impl iterator::iterator iterator::double-ended-iterator))
...
Example:
(ns-import 'iterator)
(test::assert-equal (list (join 'string 'bean) (join 'monte 'carlo)) (collect (zipper (iter (list 'string 'monte)) (iter (list 'bean 'carlo)))))
(test::assert-equal (list (join 1 2) (join 3 4)) (collect (zipper (iter (list 1 3)) (iter (list 2 4 5)))))
"
    (fst scnd)
      ((zipper-iter) :init fst scnd))
