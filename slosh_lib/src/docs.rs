...
pub const USER_FORMS: &str = "User Forms";
...
impl SloshDoc {
    ...
    pub fn new_incomplete(g: Interned, vm: &mut SloshVm, namespace: Namespace) -> DocResult<SloshDoc> {
        let sym = Value::Symbol(g);
        let slot = vm.global_intern_slot(g);
        if let Some(slot) = slot {
...
fn docs_by_section_builtin(vm: &mut SloshVm, registers: &[Value]) -> VMResult<Value> {
    if !registers.is_empty() {
        return Err(VMError::new_vm(
            "docs-by-section: takes no arguments".to_string(),
        ));
    }
    vm.pause_gc();
    let sections = get_docs_by_section(vm, false);
    let mut map = VMHashMap::with_capacity(sections.len());
    for (section, docs) in &sections {
        let key = vm.alloc_string(section.clone());
        let md: String = docs.iter().map(|d| d.as_md()).collect::<Vec<_>>().join("\n---\n\n");
        let val = vm.alloc_string(md);
        map.insert(vm, key, val);
    }
    let result = vm.alloc_map(map);
    vm.unpause_gc();
    Ok(result)
}
...
",
    ...
    add_builtin(
        env,
        "docs-by-section",
        docs_by_section_builtin,
        "Usage: (docs-by-section)

Returns a map where keys are section names and values are markdown-formatted
documentation strings for all symbols in that section. Useful for generating
documentation files or mounting docs via FUSE.
...
mod test {
    ...
    static REGEX_TEST_CASES: LazyLock<BTreeMap<(bool, &'static str), &'static str>> =
        LazyLock::new(|| {
