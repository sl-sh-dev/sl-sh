...
use crate::VarArgs;
use crate::{CharString, Codepoint};
...
fn char_int(target: String) -> LispResult<i64> {
    ...
    match count {
        ...
        1 => {
            ...
            if overflow {
                Err(LispError::new("overflow occurred in char->int adding unicode scalar values that compose provided grapheme interpreted as unsigned 32 byte integers: {:?}."))
            } else {
                Ok(int_val)
            }
        }
...
"#,
    ...
    intern_char_int(interner, data);
    intern_codepoints(interner, data);
    intern_str_float(interner, data);
}
