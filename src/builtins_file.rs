...
pub fn add_file_builtins<S: BuildHasher>(
    ...
    data.insert(
        interner.intern("cd"),
        Expression::make_function(builtin_cd, "Change directory.", root),
    );
    data.insert(
        interner.intern("fs-exists?"),
        Expression::make_function(builtin_path_exists, "Does the given path exist?", root),
    );
    data.insert(
        interner.intern("fs-file?"),
        Expression::make_function(builtin_is_file, "Is the given path a file?", root),
    );
    data.insert(
        interner.intern("fs-dir?"),
        Expression::make_function(builtin_is_dir, "Is the given path a directory?", root),
    );
    data.insert(
        interner.intern("pipe"),
        Expression::make_function(builtin_pipe, "Setup a pipe between processes.", root),
    );
    data.insert(
    ...
    data.insert(
        interner.intern("pid"),
        Expression::make_function(builtin_pid, "Return the pid of a process.", root),
    );
    data.insert(
